...
Der PK `icon_id` ist `generated by default **on null** as identity`, damit das APEX-Formular
(Automatic Row Processing) beim Anlegen den PK generieren lässt. Der Trigger
`HOM_ICONS_AUDIT` stempelt die Audit-Spalten und leitet `mime_type` aus den Magic-Bytes des
Bildes ab (PNG/JPEG/GIF/BMP/SVG) — nötig, weil das APEX-`fileUpload`-Item keine MIME-Spalte
deklarativ erfassen kann. `HOM_ICONS` ist die einzige Tabelle, die auch direkt von APEX (Form
ARP) geschrieben wird; die Icon-FKs nutzen `ON DELETE SET NULL`.
